...
import outskirts.nbt.NBT;
import outskirts.nbt.NBTUtils;
...
public final class PacketBuffer extends ByteBuf {
    ...
    public PacketBuffer writeNBT(NBT nbt) throws IOException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        NBTUtils.write(nbt, baos);
        writeByteArray(baos.toByteArray());
        return this;
    }

    public <T extends NBT> T readNBT() throws IOException {
        byte[] bytes = readByteArray();
        return NBTUtils.read(new ByteArrayInputStream(bytes));
    }



    public PacketBuffer writeVector3f(Vector3f vector) {
        writeFloat(vector.x);
        writeFloat(vector.y);
        writeFloat(vector.z);
        return this;
